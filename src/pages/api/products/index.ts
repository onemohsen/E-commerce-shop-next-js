...
export default function handler(
  ...
  const { imageQuilty, page, perPage } = new QueryParamsClass(req.query, {
    perPage: "6",
  }).getParams();

  let hasPaginate =
    req.query?.paginate && req.query.paginate == "false" ? false : true;
  ...
  let filterDataInstance = new FilterDataClass<ProductType>(
    getData(imageQuilty!),
    params,
    filters,
  );

  let { data, meta }: { data: ProductType[] | []; meta: MetaRespone } =
...
const getData: (imageQuilty: string) => ProductType[] = (
  imageQuilty = "500",
) => [
  {
    id: 1,
    ...
    rate: 1,
    title:
      "accessory1 (samsung,huawei) (Metalic,8GB Ram,Supet power,Large Memory,)",
    quantity: 2,
